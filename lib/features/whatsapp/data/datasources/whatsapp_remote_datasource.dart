  ...
  Future<List<WhatsappThreadModel>> getThreads({int page = 1}) async {
    final response = await apiClient.get(
      ...
      fromJson: (json) {
        ...
        return threadsList.map((e) => WhatsappThreadModel.fromJson(e as Map<String, dynamic>)).toList();
      },
    );
  ...
  Future<List<WhatsappMessageModel>> getThreadMessages(String threadId, {int page = 1}) async {
    final response = await apiClient.get<List<WhatsappMessageModel>>(
      ...
      fromJson: (json) {
        ...
        return messagesList.reversed.map((e) => WhatsappMessageModel.fromJson(e as Map<String, dynamic>)).toList();
      },
    );
    return response.data ?? [];
  }
