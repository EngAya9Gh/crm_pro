...
- **GET /settings/statuses** -> Output:
```json
{
  "success": true,
  "data": [
    { "id": 1, "name": "جديد", "color": "#FF0000", "order": 1, "weight": 0, "is_default": true }
  ]
}
```
- **POST /settings/statuses** -> Body: `{ "name": "...", "color": "#...", "order": 1, "weight": 50 }` -> Returns Created Object.

### 3.2 المصادر (Sources)
- **GET /settings/sources** -> Output:
```json
{
  "success": true, 
  "data": [ { "id": 1, "name": "Facebook", "is_active": true } ]
}
```
- **POST /settings/sources** -> Body: `{ "name": "Instagram", "is_active": true }` -> Returns Created Object.

### 3.3 السلوكيات (Behaviors)
- **GET /settings/behaviors** -> Output:
```json
{
  "success": true, 
  "data": [ { "id": 1, "name": "مهتم", "color": "#00FF00" } ]
}
```
- **POST /settings/behaviors** -> Body: `{ "name": "...", "color": "..." }`

### 3.4 أسباب الاستبعاد (Invalid Reasons)
- **GET /settings/invalid-reasons** -> Output:
```json
{
  "success": true, 
  "data": [ { "id": 1, "name": "الرقم خطأ" } ]
}
```
- **POST /settings/invalid-reasons** -> Body: `{ "name": "..." }`

### 3.5 المناطق (Regions)
- **GET /settings/regions** -> Output:
```json
{
  "success": true, 
  "data": [ { "id": 1, "name": "الرياض" } ]
}
```
- **POST /settings/regions** -> Body: `{ "name": "..." }`

### 3.6 المدن (Cities)
- **GET /settings/cities?region_id=1** -> Output:
```json
{
  "success": true, 
  "data": [ { "id": 5, "name": "العليا", "region_id": 1 } ]
}
```
- **POST /settings/cities** -> Body: `{ "name": "...", "region_id": 1 }`

### 3.7 التاجز (Tags)
- **GET /settings/tags** -> Output:
```json
{
  "success": true, 
  "data": [ { "id": 1, "name": "VIP", "color": "#GOLD" } ]
}
```
- **POST /settings/tags** -> Body: `{ "name": "...", "color": "..." }`

### 3.8 المنتجات (Products)
- **GET /settings/products** -> Output:
```json
{
  "success": true, 
  "data": [ { "id": 1, "name": "استشارة", "price": 500, "is_active": true } ]
}
```
- **POST /settings/products** -> Body: `{ "name": "...", "price": 500, "is_active": true }`

### 3.9 تاجز الفواتير (Invoice Tags)
- **GET /settings/invoice-tags** -> Output:
```json
{
  "success": true, 
  "data": [ { "id": 1, "name": "مدفوع", "color": "#GREEN" } ]
}
```
- **POST /settings/invoice-tags** -> Body: `{ "name": "...", "color": "..." }`

### 3.10 أنواع التعليقات (Comment Types)
- **GET /settings/comment-types** -> Output:
```json
{
  "success": true, 
  "data": [ { "id": 1, "name": "Call", "icon": "phone", "color": "#blue" } ]
}
```
- **POST /settings/comment-types** -> Body: `{ "name": "...", "icon": "...", "color": "..." }`

### 3.11 الفرق (Teams)
- **GET /settings/teams** -> Output:
```json
{
  "success": true, 
  "data": [ 
    { 
      ...
      "name": "المبيعات", 
      "description": "...", 
      "roles": [ { "id": 1, "name": "مدير" } ] 
    } 
  ]
}
```
- **POST /settings/teams** -> Body: `{ "name": "...", "description": "..." }`

### 3.12 الأدوار (Roles)
- **GET /settings/roles?team_id=1** -> Output:
```json
{
  "success": true, 
  "data": [ 
    { 
      ...
      "team_id": 1,
      "is_default": true,
      "permissions": [ { "id": 1, "name": "read_clients" } ]
    } 
  ]
}
```
- **POST /settings/roles** -> Body: `{ "name": "...", "team_id": 1, "permissions": [1, 2] }`
